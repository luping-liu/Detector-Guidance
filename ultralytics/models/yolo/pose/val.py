...
from ultralytics.utils.metrics import OKS_SIGMA, MY_SIGMA, PoseMetrics, box_iou, kpt_iou
...
class PoseValidator(DetectionValidator):
    ...
    def init_metrics(self, model):
        ...
        if self.kpt_shape == [17, 3]:
            self.sigma = torch.from_numpy(OKS_SIGMA).to(self.device)
        elif self.kpt_shape == [18, 3]:
            self.sigma = torch.from_numpy(MY_SIGMA[:18]).to(self.device)
        elif self.kpt_shape == [60, 3]:
            self.sigma = torch.from_numpy(MY_SIGMA).to(self.device)
        else:
