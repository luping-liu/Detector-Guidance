...
class HungarianMatcher(nn.Module):
    ...
    def forward(self, pred_bboxes, pred_scores, gt_bboxes, gt_cls, gt_groups, masks=None, gt_mask=None):
        ...
        if self.use_fl:
            neg_cost_class = (1 - self.alpha) * (pred_scores ** self.gamma) * (-(1 - pred_scores + 1e-5).log())
            pos_cost_class = self.alpha * ((1 - pred_scores) ** self.gamma) * (-(pred_scores + 1e-5).log())
...
def get_cdn_group(batch,
    ...
    if box_noise_scale > 0:
        ...
        dn_bbox = torch.logit(dn_bbox, eps=1e-5)  # inverse sigmoid
