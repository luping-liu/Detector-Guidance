...
hand = [0.029, 0.022, 0.035, 0.037, 0.047, 0.026, 0.025, 0.024, 0.035, 0.018, 0.024, 0.022, 0.026, 0.017,
        0.021, 0.021, 0.032, 0.02, 0.019, 0.022, 0.031]
mm2open = [1, 6, 7, 9, 11, 6, 8, 10, 13, 15, 17, 12, 14, 16, 3, 2, 5, 4]
body_mm = [body[i-1] for i in mm2open]
MY_SIGMA = np.array(body_mm + hand + hand)


def bbox_ioa(box1, box2, iou=False, eps=1e-5):
    """
...
def box_iou(box1, box2, eps=1e-5):
    """
...
def bbox_iou(box1, box2, xywh=True, GIoU=False, DIoU=False, CIoU=False, eps=1e-5):
    """
...
def mask_iou(mask1, mask2, eps=1e-5):
    """
    Calculate masks IoU.
...
def kpt_iou(kpt1, kpt2, area, sigma, eps=1e-5):
    """
...
class ConfusionMatrix:
    ...
    def plot(self, normalize=True, save_dir='', names=(), on_plot=None):
        ...
        array = self.matrix / ((self.matrix.sum(0).reshape(1, -1) + 1E-5) if normalize else 1)  # normalize columns
