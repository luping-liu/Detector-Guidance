...
class LatentDiffusion(DDPM):
    ...
    def forward(self, x, c, *args, **kwargs):
        ...
        t = torch.randint(0, self.num_timesteps - 180, (x.shape[0],), device=self.device).long()  # yolo_100
        # t = torch.ones_like(t) * 50
    ...
    def p_losses(self, x_start, cond, t, noise=None, reweight=None):
        noise = default(noise, lambda: torch.randn_like(x_start))
        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)
        model_output = self.apply_model(x_noisy, t, cond)
        x_start_pred = self.predict_start_from_noise(x_noisy, t, model_output)
        ...
        if reweight is None:
            reweight = 1.0
        loss_simple = self.get_loss(model_output, target, mean=False) * reweight  # .mean([1, 2, 3])
        loss_dict.update({f'{prefix}/loss_simple': loss_simple.mean()})
        loss_dict.update({f'{prefix}/x_start_pred': x_start_pred})
        loss_dict.update({f'{prefix}/x_noisy': x_noisy})
        loss_dict.update({f'{prefix}/timestep': t.float().mean()})
