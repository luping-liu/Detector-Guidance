...
# sys.path.append('../')
# sys.path.append('../annotator/ppocr')
from ldm.modules.diffusionmodules.util import conv_nd, linear
from torch import Tensor

from .recognizer import create_predictor, TextRecognizer
...
def get_recog_emb(encoder, img_list):
    ...
    _img_list = [torch.tensor(img * 255) for img in img_list]  # .to('cuda', torch.float16)
    encoder.predictor.eval()
    preds, preds_neck = encoder.pred_imglist(_img_list, show_debug=False)
    return preds_neck, preds
...
class EmbeddingManager(nn.Module):
    def __init__(
        ...
        super().__init__()
        # get_token_for_string = partial(get_clip_token_for_string, embedder)
        # token_dim = 768
        ...
        # rec_model_dir = "stablediffusion/annotator/ckpts/ppv3_rec_anytext.pth"
        # predictor = create_predictor(rec_model_dir)
        # args = AttrDict()
        # args.rec_image_shape = "3, 48, 320"
        # args.rec_char_dict_path = r"stablediffusion/annotator/anytext/ppocr_keys_v1.txt"
        # args.rec_batch_num = 4
        # self.recog = TextRecognizer(args, predictor)

        # self.placeholder_token = get_token_for_string(placeholder_string)

    def register_recog(self, recog):
        self.recog = recog
        self.get_recog_emb = partial(get_recog_emb, self.recog)

    def encode_text(self, gline_list):
        ...
        if self.emb_type == 'ocr':
            recog_emb, preds = self.get_recog_emb(gline_list)
            enc_glyph = self.proj(recog_emb.reshape(recog_emb.shape[0], -1))
            # import pdb; pdb.set_trace()  # debug
            # preds_all = preds.softmax(dim=2)
            # order, idx = self.recog.decode(preds_all[0])
            # text = self.recog.get_text(order)
            # print(text)
...
if __name__ == "__main__":
    ...
    embedder = SimpleTokenizer()
    embed_manager = EmbeddingManager(embedder, )
    rec_model_dir = "stablediffusion/annotator/ckpts/ppv3_rec_anytext.pth"
    predictor = create_predictor(rec_model_dir)
    args = AttrDict()
    args.rec_image_shape = "3, 48, 320"
    args.rec_char_dict_path = r"stablediffusion/annotator/anytext/ppocr_keys_v1.txt"
    args.rec_batch_num = 4
    embed_manager.recog = TextRecognizer(args, predictor)

    dataset = HunyuanDatasetStream(img_size=1024, split="train", data_type=["text-4m"])
    for i in tqdm(range(len(dataset))):
        ...
        pdb.set_trace()
        embed_manager.encode_text(item['txt_list'])
        print('pass')