...
import time
from annotator.anytext.RecModel import RecModel
import torch
...
def create_predictor(model_dir=None, model_lang='ch', is_onnx=False):
    model_file_path = model_dir if model_dir is not None else "./weights/ppv3_rec.pth"
    ...
    if is_onnx:
        ...
        rec_config = AttrDict(
            ...
            neck=AttrDict(type='SequenceEncoder', encoder_type="svtr", dims=64, depth=2, hidden_dims=120, use_guide=True),
            head=AttrDict(type='CTCHead', fc_decay=0.00001, out_channels=n_class, return_feats=True)
        )
...
def main():
    ...
    args.rec_char_dict_path = r"./weights/ppocr_keys_v1.txt"
